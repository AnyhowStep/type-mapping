...
Object.appendChain = function(oChain : any, oProto : any) {
    ...
    let oReturn = oChain instanceof this ? oChain : new oChain.constructor(oChain);
    let o2nd = oReturn;
    let oLast = oReturn;
...
export class JSBI extends Array<number> {
    ...
    private static readonly __kBitsPerCharTableShift = 5;
    private static readonly __kBitsPerCharTableMultiplier = 1 << JSBI.__kBitsPerCharTableShift;
    private static readonly __kConversionChars = '0123456789abcdefghijklmnopqrstuvwxyz'.split('');
    private static readonly __kBitConversionBuffer = new ArrayBuffer(8);
    private static readonly __kBitConversionDouble = new Float64Array(JSBI.__kBitConversionBuffer);
    private static readonly __kBitConversionInts = new Int32Array(JSBI.__kBitConversionBuffer);
    ...
    static remainder(x : JSBI, y : JSBI) {
        if (y.length === 0) throw new RangeError('Division by zero');
    ...
    static __fromString(string : string, radix = 0) : JSBI|null {
        let sign = 0;
        ...
        const result : JSBI = new JSBI(resultLength, false);
    ...
    static __absoluteAddOne(x : JSBI, sign : boolean, result : JSBI|null = null) : JSBI {
    ...
    static __absoluteAnd(x : JSBI, y : JSBI, result : JSBI|null = null) : JSBI {
    ...
    static __absoluteAndNot(x : JSBI, y : JSBI, result : JSBI|null = null) : JSBI {
        const xLength = x.length;
    ...
    static __absoluteOr(x : JSBI, y : JSBI, result : JSBI|null = null) : JSBI {
        let xLength = x.length;
        let yLength = y.length;
        let numPairs = yLength;
        if (xLength < yLength) {
    ...
    static __absoluteXor(x : JSBI, y : JSBI, result : JSBI|null = null) : JSBI {
        let xLength = x.length;
        let yLength = y.length;
        let numPairs = yLength;
        if (xLength < yLength) {
    ...
    static __absoluteDivLarge(dividend : JSBI, divisor : JSBI, wantQuotient : boolean, wantRemainder : boolean) : JSBI|{ quotient : JSBI, remainder : JSBI } {
        ...
        if (wantQuotient && q != null) return q;
    ...
    static __toPrimitive(obj : any, hint = 'default') : any {
        ...
        const exoticToPrim = Symbol.toPrimitive == undefined ? undefined : obj[Symbol.toPrimitive];
        if (exoticToPrim) {
    ...
    static __toNumeric(value : string|number|JSBI) {

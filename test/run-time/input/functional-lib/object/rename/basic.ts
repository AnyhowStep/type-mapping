...
tape(__filename, t => {
    const f = tm.rename("a", "a2", tm.finiteNumber());

    t.deepEqual(f("x", { a : 2 }), { a2 : 2 });
    t.deepEqual(f("x", { a : 2, a2 : 3 }), { a2 : 3 });
    t.deepEqual(f("x", { a2 : 4 }), { a2 : 4 });
...
tape(__filename, t => {
    const f = tm.rename("a", "a2", tm.orUndefined(tm.finiteNumber()));
    ...
    t.deepEqual(f("x", { a : 5, a2 : undefined }), { a2 : undefined });
    ...
    /**
        ...
        The objects pass because `a` or `a2` is required during compile-time

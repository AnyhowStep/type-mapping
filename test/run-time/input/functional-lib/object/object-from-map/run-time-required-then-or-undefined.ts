...
tape(__filename, t => {
    const dst = tm.fields({
        a2 : tm.orUndefined(tm.runTimeRequired(tm.finiteNumber())),
    });
    ...
    t.deepEqual(f("x", { a : undefined }), { a : undefined });
    t.deepEqual(f("x", { a : 42 }), { a : 42 });

    t.false(tm.tryMap(f, "x", { a2 : undefined } as any).success);
    t.false(tm.tryMap(f, "x", { a2 : 42 } as any).success);

    t.deepEqual(f("x", { a : undefined, a2 : undefined }), { a : undefined });
    t.deepEqual(f("x", { a : undefined, a2 : 42 }), { a : undefined });
    t.deepEqual(f("x", { a : 42, a2 : undefined }), { a : 42 });
    t.deepEqual(f("x", { a : 32, a2 : 42 }), { a : 32 });

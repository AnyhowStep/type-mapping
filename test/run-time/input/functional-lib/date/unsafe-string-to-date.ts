...
tape(__filename, t => {
    const f = tm.unsafeStringToDate();

    const d = new Date();
    t.true(f("x", d) == d);
    t.deepEqual(f("x", "2019-01-01").getTime(), new Date("2019-01-01").getTime());

    t.false(tm.tryMap(f, "x", "2019-01-50" as any).success);
    t.deepEqual(f("x", "1").getTime(), new Date("1").getTime());

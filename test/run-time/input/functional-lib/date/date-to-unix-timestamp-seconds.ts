...
tape(__filename, t => {
    const f = tm.dateToUnixTimestampSeconds();

    const d = new Date();
    t.true(f("x", d) == Math.floor(d.getTime()/1000));
    t.deepEqual(f("x", new Date(2000)), 2);

    t.deepEqual(f("x", new Date(1)), 0);
    t.deepEqual(f("x", new Date(1000)), 1);
